...
TMP_ZIP = "/tmp/prepare_ghost_theme.zip"

def run_cmd(client, cmd, sudo=False):
    print(f"\n🚀 [Executando na VM Azure hmt]: {cmd}")
...
def prepare_local_zip():
    print("📦 Preparando pacote ZIP do tema...")
    if os.path.exists(TMP_ZIP):
        os.remove(TMP_ZIP)
    if os.path.exists(LOCAL_THEME_FOLDER):
        shutil.make_archive("/tmp/prepare_ghost_theme", "zip", LOCAL_THEME_FOLDER)
    else:
        shutil.copy(LOCAL_THEME_ZIP, TMP_ZIP)
    print("✅ Pacote ZIP do tema preparado em /tmp/prepare_ghost_theme.zip")

def connect_with_retry(max_retries=12, delay=10):
    client = paramiko.SSHClient()
    client.set_missing_host_key_policy(paramiko.AutoAddPolicy())

    for attempt in range(1, max_retries + 1):
        print(f"📡 Tentando conectar via SSH a {USER}@{HOST} (Tentativa {attempt}/{max_retries})...")
        try:
            client.connect(hostname=HOST, username=USER, password=PASS, timeout=10)
            print("✅ Conexão SSH estabelecida com sucesso!")
            return client
        except Exception as e:
            print(f"⏳ VM offline ou ligando... ({e}). Aguardando {delay}s...")
            time.sleep(delay)

    print("❌ VM Azure indisponível após várias tentativas.")
    return None

def main():
    prepare_local_zip()

    client = connect_with_retry(max_retries=15, delay=10)
    if not client:
        print("💡 Caso a VM esteja desligada no Portal Azure, ligue-a em https://portal.azure.com e me avise!")
        sys.exit(0)

    try:
        # 1. Atualizar e preparar pacotes
        ...
        # 2. Node.js LTS v20 + PM2 + Ghost-CLI
...
"""
        ...
        print("\n" + "="*80)
        print("🎉 PREPARAÇÃO E INSTALAÇÃO DO GHOST CONCLUÍDAS COM SUCESSO!")
        print(f"🖥️ Portal Azure VM: https://portal.azure.com/#@cienciamsn.onmicrosoft.com/resource/subscriptions/4c485838-8ddd-485c-a351-b50d55a4184a/resourceGroups/hmt-pro/providers/Microsoft.Compute/virtualMachines/hmt/overview")
        print(f"🌐 Website Oficial: https://{DOMAIN}")
        print(f"🛠️ Admin Ghost: https://{DOMAIN}/ghost/")
        print("="*80)
