...
const ALL_PERMISSIONS: PermissionItem[] = [
  { id: "view_dashboard", label: "📊 Visualizar Dashboard & Métricas", category: "Geral" },
  {
    ...
    label: "💬 Atender & Gerenciar Conversas",
  ...
  {
    id: "manage_flowbuilder",
    label: "⚡ Criar & Editar Fluxos FlowBuilder IA",
    category: "Automação",
  },
  {
    id: "manage_automations",
    label: "🤖 Configurar Robôs & Regras de Atendimento",
    category: "Automação",
  },
  ...
  { id: "manage_courses", label: "🎓 Gerenciar Cursos & Videoaulas", category: "Educação" },
  {
    ...
    label: "🪝 Configurar Hotmart, ABACS, Kiwify & Meta",
...
export function PermissionsPage() {
  ...
  const selectedRole: RoleItem = (roles.find((r) => r.id === selectedRoleId) ||
    roles[0] ||
    DEFAULT_ROLES[0]) as RoleItem;
      ...
      >
        <div>
          ...
          <p className="muted" style={{ marginTop: -8 }}>
            Defina o que cada perfil de atendente, supervisor ou administrador pode visualizar e
          ...
          {roles.map((role) => (
            <button
              ...
              style={{
                ...
                background:
                  selectedRoleId === role.id ? "rgba(109, 40, 217, 0.12)" : "var(--panel)",
                color: "var(--text)",
                textAlign: "left",
                cursor: "pointer",
              }}
            >
              <div
                style={{
                  fontWeight: 700,
                  fontSize: 14,
                  color: selectedRoleId === role.id ? "var(--accent)" : "var(--text)",
                }}
              >
                {role.name}
              </div>
              ...
              <span
                className="tag"
                style={{ marginTop: 8, fontSize: 10, background: "var(--panel2)" }}
              >
